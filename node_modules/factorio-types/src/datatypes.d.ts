...
interface EnergySource {
	...
	usage_priority: "primary-input" | "primary-output" | "secondary-input" | "secondary-output" | "tertiary" | "solar" | "lamp",
}
...
interface ProductPrototypeTable {
    type?: 'item' | 'fluid'
}

type ProductPrototype = FilteredExtension<ProductPrototypeTable,
[
	ItemProductPrototypeTable,
...
interface ItemProductPrototypeTable extends ProductPrototypeTable {
    type?: 'item'
    name: string
    amount?: number
    probability?: number
...
interface FluidProductPrototype extends ProductPrototypeTable {
    type: 'fluid'
    name: string
    probability?: number
    amount?: number
...
type IngredientPrototype = FilteredExtension<IngredientPrototypeTable,
[
	ItemProductPrototypeTable,
	FluidProductPrototype
]
...
type Transitions = FilteredExtension<TransitionsBase, [
	{
...
type VariantTransitions = Transitions & {
	main: Array<TileSprite & {
		/**
		 * Only powers of 2 from 1 to 128 can be used. Square size of the tile arrangement this animation is used for. Used to calculate the width and height of the animation which cannot be set directly. (width or height) = size * 32 / scale
		 */
		size: number;
		/**
		 * Default: 1.0 - Probability of 1x1 (size = 1) version of tile must be 1.
		 */
		probability?: number;
		weights?: number[];
	}>
