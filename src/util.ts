...
const eventList: Map<defines.events | defines.events[] | string, EventFunc[]> = new Map();
/**
 ...
 */
export function DefineEvent<T extends keyof AllEvents>(event: T, callback: (this: void, event: AllEvents[T]) => void) {
	if (!eventList.has(event)) eventList.set(event, []);
	eventList.get(event)!.push(callback as EventFunc);
}

const t = DefineEvent(defines.events.on_area_cloned, event => {});
